...
class Parser {
  ...
  buildAtomFeed(xmlObj) {
    ...
    if (xmlObj.feed.author && xmlObj.feed.author.length && xmlObj.feed.author[0].name && xmlObj.feed.author[0].name[0]) {
      feed.author = xmlObj.feed.author[0].name[0]
    }
  ...
  parseItemAtom(entry) {
    ...
    if (entry.category && entry.category.length) {
      item.categories = entry.category.map(e => {
        return e && e.$ && e.$.term
      });
    }
  ...
  parseItemRss(xmlItem, itemFields) {
    ...
    if (xmlItem.category && xmlItem.category.length) {
      item.categories = xmlItem.category.map(e => e._ || e);
    }
